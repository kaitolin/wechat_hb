...
class AuthAction extends Action {
    ...
    public function _initialize () {
		...
		if(isset($_REQUEST["code"])){
            ...
            {
	            $user = $this->getUserInfo($res["openid"]);
	        	if($user === false)
	        	{
	        		$this->error("受权用户信息失败");
	        	}
    ...
    * 微信初始化认证
    **/
    private function innerWechatInitAuth(){
    ...
    **/
	...
	protected function getUserInfo($openid){
		$wtoken =$this->get_weixin_access_token();
	    $url = "https://api.weixin.qq.com/cgi-bin/user/info?access_token=".$wtoken."&openid=".$openid;
        $Token = $this->curlGetInfo($url);
        $data = json_decode($Token, true);
        if($data['errcode']){
            Log::write('平台获取关注者详细信息错误'.$Token);
            return false;
        }
		return $data;
	}
